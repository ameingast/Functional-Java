...
public class PairTest {
    private final Pair<String, Integer> pair = new Pair<String, Integer>("foo",
        123);
    ...
    public void testComparator() {
        ...
        if (pairs.add(p2) && pairs.add(p1)) {
            Collections.sort(pairs,
                new PairFirstElementComparator<Integer, String>());
            assertEquals(p1, pairs.get(0));
            assertEquals(p2, pairs.get(1));

            Collections.sort(pairs,
                new PairSecondElementComparator<Integer, String>());
