...
public class ZipTest extends BaseTest {
    ...
    public void testGenericIntegerZip() {
        assertArrayEquals(iterateZip(data, data).toArray(), getZip().apply(
            data, data).toArray());
    }

    @Test
    public void testFirstListLonger() {
        assertArrayEquals(iterateZip(as, bs).toArray(), getZip().apply(as, bs)
            .toArray());
    }

    @Test
    public void testSecondListLonger() {
        assertArrayEquals(iterateZip(bs, as).toArray(), getZip().apply(bs, as)
            .toArray());
    }

    private List<Pair<Integer, Integer>> iterateZip(final List<Integer> as,
        final List<Integer> bs) {
        final List<Pair<Integer, Integer>> zs = new ArrayList<Pair<Integer, Integer>>();
        int i = 0;

        for (i = 0; i < as.size(); i++)
            zs.add(new Pair<Integer, Integer>(as.get(i), i < bs.size() ? bs
                .get(i) : null));
        for (; i < bs.size(); i++)
            zs.add(new Pair<Integer, Integer>(i < as.size() ? as.get(i) : null,
                bs.get(i)));
        return zs;
    }
}
