...
public class ParallelMapFold<A, B, C> extends MapFold<A, B, C> {
    ...
    public ParallelMapFold(final MapFunctor<A, B> mapFunctor,
    ...
    public C apply(final List<A> as, final C c) {
        try {
            return MapWorker.start(workerCount, getFolderAggregator(as.size(),
                c), as, mapFunctor);
    ...
    protected Aggregator<B, C> getFolderAggregator(final Integer itemCount,
        final C e) {
