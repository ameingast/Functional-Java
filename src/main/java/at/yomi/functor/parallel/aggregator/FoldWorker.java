...
public class FoldWorker<A, B> extends AbstractWorker<A, B> {
    ...
    public static <A> A start(final Integer workerCount,
        final ListAggregator<A> aggregator, final List<A> as,
        final FoldFunctor<A, A> foldFunctor, final A e)
        throws InterruptedException {
        final List<AbstractWorker<A, A>> workers = new ArrayList<AbstractWorker<A, A>>(
            workerCount);
    ...
    public FoldWorker(final Aggregator<B, ?> aggregator,
    ...
    @Override
    public void run() {
        aggregator.add(new FoldSecond<Integer, A, B>(functor).apply(items, e),
            items.size());
    }
}
